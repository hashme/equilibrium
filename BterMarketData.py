...
import json
import operator
import bterapi.bterconnection
import bterapi.common
import threading
...
def getMarketData( pair, arc_dict):
        ...
        connection = bterapi.bterconnection.BTERConnection()
        try:
            depth = bterapi.common.validateResponse(connection.makeJSONRequest('/api/1/depth/%s' % pair, method='GET'))
...
def getTickerList():
    try:
        return bterapi.bterconnection.BTERConnection().makeJSONRequest("/api/1/pairs", method="GET")
    except:
        print 'wtf'
