...
def get_data():
    ...
    try:
        ...
        poloniex_data = {}
    data = dict(coinse_data.items()+cryptsy_data.items()+bter_data.items()+poloniex_data.items())
    return data
...
def executeOnce():
    ...
    forbidden_exchanges = ['poloniex','bter']
    data = get_data()
    path, return_on_cycle = get_path(data)
    volume = get_volume(path, data, return_on_cycle)
    print '(!) Found cycle!'
    print path
    print '(!) Multiplicative constant of '+str(return_on_cycle)+'.'
    print '(!) '+str(volume)+' orders open.'
    print '(!) Profit of '+str(float(volume)*float(return_on_cycle))+' in '+path[0].split('_')[0]+' if this cycle is exploited!'
    if 1==1:return
    for k in range(20):
        path, return_on_cycle = get_path(data)
        if return_on_cycle > 1.1:
            for item in path:
    ...
    print '(.) Starting with '+volume+' of '+path[0].split('_')[0]+'.'
    for i in range(len(path)-1):
        frm = path[i].split('_')[0]
        to = path[i+1].split('_')[0]
        price = float(data[path[i].split('_')[0]][path[i+1].split('_')[0]][0])
        print '(.) Trading '+volume+' of '+frm+' for '+to+' at '+str(price)+'.'
        executor(frm,to,volume,price)
        volume /= price
        print '(@) Successful trade! Now have '+volume+' of '+to+'.'
    print '(@) Ended with '+volume+' of '+path[0].split('_')[0]+'!!!'
